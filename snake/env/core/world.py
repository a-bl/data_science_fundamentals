...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = tuple(np.random.randint(SNAKE_SIZE, self.size[0]-SNAKE_SIZE, size=2))
    ...
    def init_food(self):
        ...
        available_food_positions = self.available_food_positions - set(self.snake.blocks)
        if not self.custom:
            # Choose a random position from available
            chosen_position_list = random.sample(self.available_food_positions, 1)
            chosen_position = (chosen_position_list[0][0], chosen_position_list[0][1])
        else:
